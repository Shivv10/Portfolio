...
const About: React.FC<AboutProps> = ({ navigateTo, darkMode, setDarkMode }) => {
  ...
  // The "Dream List" for the chaotic energy section
  const dreams = [
    { icon: <Code size={16}/>, text: "Tech Savy" },
    { icon: <Plane size={16}/>, text: "Traveler" },
    { icon: <Dumbbell size={16}/>, text: "Fitness Entusiast" },
  ...
  style={{ objectPosition: "50% 50%" }} // smaller Y => moves visible area up
/>

             <div className="w-full h-full flex items-center justify-center text-slate-400">
                <span className="text-xs uppercase tracking-widest font-bold">Profile Pic</span>
             </div>
          </div>
  ...
  <div className="col-span-2 row-span-2 relative overflow-hidden rounded-2xl">
    <img
      src="src/img/banff.jpg"
      alt="Nature shot"
      className="absolute inset-0 w-full h-full object-cover scale-110"
      style={{ objectPosition: "50% 40%" }} // lower 2nd number = show more top
    />
  </div>
...
<div className="aspect-video relative overflow-hidden rounded-2xl bg-slate-200 dark:bg-slate-800">
  <img
    src="src/img/cricket.JPG"
    alt="cricket"
    className="absolute inset-0 w-full h-full object-cover"
    style={{ objectPosition: "50% 50%" }} // move crop up if needed
  />
</div>
          </div>
        </motion.div>
